...
rownames(table_of_mean) <- row_names
...
for (grade in list_of_grades){
  ...
  for (i in 1:length(row_names)){
    if (substr(row_names[i], 1, 1)=="o"){
      ...
      table_of_mean[[paste0("grade",grade)]][i] <- round(mean(w[,the_index]),4)
      ...
      table_of_mean[[paste0("grade",grade)]][i] <- round(mean(theta[the_index,permutation_index,]),4)
    }
  }
}

all_objects <- ls()
objects_to_keep <- c("table_of_mean")
objects_to_remove <- setdiff(all_objects, objects_to_keep)
rm(list = objects_to_remove)

save(table_of_mean,file="mean_for_all_grades_multivariate_Bernoulli.Rdata")

latex_code <- xtable(table_of_mean, caption = "mean for parameters from 20000 iterations using Algorithm.1 when fitting
